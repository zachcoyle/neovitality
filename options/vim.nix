    ...
    in
    {
      ...
      vim.configRC = ''
        ...
        ${wrapLuaConfig ''
            ...
            -- temporary but I need it for work --

            local eslint = {
              lintCommand = "${pkgs.nodePackages.eslint_d}/bin/eslint_d -f unix --stdin --stdin-filename ''${INPUT}",
              lintStdin = true,
              lintFormats = {"%f:%l:%c: %m"},
              lintIgnoreExitCode = true,
              formatCommand = "${pkgs.nodePackages.eslint_d}/bin/eslint_d --fix-to-stdout --stdin --stdin-filename=''${INPUT}",
              formatStdin = true
            }
